...
sess = mygui.data[1]
outFile = "C:/Users/pain_kosek/Documents/POADOS/sponPain_fmri_pain/data/" + expName + "_" + subj_id + "_" + sess + ".csv"
...
msg = visual.TextStim(win, color='LightGray', text='väntar på magnetkamera...')   # text='waiting for scanner...'
...
fixSpot = visual.GratingStim(win, tex="none", mask="circle", size=(0.08, 0.12), color='red', autoLog=False)
...
vas = visual.RatingScale(win,
                         ...
                         low=0,                 # minimum
                         high=100,              # maximum
                         textSize=1.4,          # applied to all text elements (anchors, scale)  1.3
...
while ratingTimer.getTime() > 0:  # until timer is at 0
    ...
    currentMousePos = vas._getMarkerFromPos(vm.getPos()[0])  # grabs current mouse position (rating)
    currentTime = globalClock.getTime()                      # grabs time since globalClock was created

    if currentMousePos != recordedMousePos[k-1, 0]:  # if the mouse has been moved since last recorded position

        recordedMousePos[k, 0] = currentMousePos   # log new rating
        recordedMousePos[k, 1] = currentTime       # log time when mouse was moved to new mouse position
