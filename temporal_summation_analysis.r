...
for(j in 1:length(pthdir)) {   # loop over subjects in directory
  ...
  Repetition<-character(); Location<-character()
  ...
  if (length(file.names) == 5 & pthdir[j] == "1018") {
    ...
    morecolss = ggthemes::canva_palettes$'Warm naturals'; morecolss = morecolss[1] # only one back TS
  }
  ...
  print(ggplot(data=printdfh, aes(x=round(Time, digits = 1), y=Rating, ymin=0, ymax=70, group = Repetition)) +
        geom_line(aes(color=Repetition), size=1) + xlab("Time (s)") + ylab("VAS") +
        ...
        ggtitle(paste(kk[1], "scores:", kk[2], "hand")) + scale_color_manual(values=colss) + theme_minimal())
    ...
    print(ggplot(data=printdfb, aes(x=round(Time, digits = 1), y=Rating, ymin=0, ymax=70, group = Repetition)) +
           ...
           ggtitle(paste(kk[1], "scores:", kk[2], "back")) + scale_color_manual(values=morecolss) + theme_minimal())
    Sys.sleep(3)


    ## calculate wind up ratio (WUR)
    # rating of single stimulus, "first stim"
    firsts <- ind.file %>% filter(Cond == "first") %>% droplevels  # create df for wind-up ratio (first, stop stim)
    ...
    series <- series %>% group_by(id,Cond,Location,Repetition) %>% summarise(Rating = max(Rating))
...
ggplot(wurL, aes(x = group, y = WUR, fill=Location)) + geom_bar(stat = "identity")
