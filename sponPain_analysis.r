...
for(j in 1:length(file.names)) {
  ...
  if (tail(file$Time, n=1) < 600) {   # make sure there are values until 600 frames, right-pad last rating
    ...
    tailfile <- data.frame(paddedid, paddedtailtime, paddedtailscore); colnames(tailfile)<-c("id", "Time", "Rating")
  ...
  if (head(file$Time, n=1) > 1) {
    ...
    headfile <- data.frame(paddedid, paddedheadtime, paddedheadscore); colnames(headfile)<-c("id", "Time", "Rating")
  ...
  # plot data
  if (id[1] < 4000) {
  print(ggplot(data=afile, aes(x=Time, y=Rating, ymin=0, ymax=50, xmin=0, xmax=600)) + geom_line(size=2) +
          theme_minimal() + xlab("Time") + ylab("Rating") + ggtitle(paste("spontaneous pain ratings: ", kk[2])))
  } else {
    print(ggplot(data=afile, aes(x=Time, y=Rating, ymin=0, ymax=50, xmin=0, xmax=600)) + geom_line(size=2) +
            theme_minimal() + xlab("Time") + ylab("Rating") + ggtitle(paste("spontaneous unpleasantness ratings: ", kk[2])))
  }

  #  Sys.sleep(3) # pause for 3 sec to look at the plot
}

inx <- out.file %>%  group_by(id) %>% summarise_at(vars(Rating), list(mean=mean, sd=sd, med=median, max=max))
